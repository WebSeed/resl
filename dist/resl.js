...
},{"./Loader":1,"./constants":2}],4:[function(require,module,exports){
var get = require('./get')
var constants = require('./constants')
var Loader = require('./Loader')
...
module.exports = function resl (config) {
  ...
  checkType(config, parameters.config, 'config')
  ...
  var loaders = {
    text: function (request) {
      return get(request, assets, abort, onComplete, notifyProgress)
    },
  ...
  var pending = Object.keys(manifest).map(function (name) {
    ...
    checkType(request, parameters.manifest, 'asset "' + name + '"')
    ...
    if ('parser' in request) {
      if (typeof request.parser === 'function') {
        ...
        checkType(parser, parameters.parser, 'parser for asset "' + name + '"')
        if (!('onData' in parser)) {
...
},{"./Loader":1,"./constants":2,"./get":3}]},{},[4])(4)
});